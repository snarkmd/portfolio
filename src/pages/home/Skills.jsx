...
// Task 5: removed the unused imports and unused destructured hook value from the skills module.
// Task 9: converted Skills to forwardRef and duplicated ticker slides so Swiper loop mode has enough items.
const Skills = forwardRef((props, ref) => {
  const containerRef = useRef(null);
  const tickerTools = useMemo(
    () => [...skills.flatMap((skill) => skill.tools), ...skills.flatMap((skill) => skill.tools)],
    []
  );
    ...
    <>
      ...
      >
        {tickerTools.map((tool, index) => (
          <SwiperSlide key={`${tool.name}-${index}`} style={{ width: "fit-content" }}>
